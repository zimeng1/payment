package com.mc.payment.gateway.channels.fireblocks.config;
...
import com.mc.payment.gateway.channels.fireblocks.model.constant.FireBlocksConstant;
...
public class FireBlocksConfig {
    ...
    public ConfigurationOptions getConfigurationOptions() {
        ...
        String secretKeyName = FireBlocksConstant.SECRET_PATH + "_" + env;
        String secretKey = readPrivateKey("fireblocks_secret_" + env);
        if (StrUtil.isBlank(secretKey)) {
            log.debug("[getConfigurationOptions] 通过容器/app/key目录没有获取到,则从resources配置中心获取");
            // 如果通过容器/app/key目录没有获取到,则从resources配置中心获取,通过容器获取env一定有值,所以一开始可用不用判断
            if (StrUtil.isNotBlank(env)) {
                secretKey = getFileContent(secretKeyName + ".key");
            } else {
    ...
    public void resetClientByUriChange() {
        if (fireblocks == null) {
            fireblocks = new Fireblocks(getConfigurationOptions());
        } else {
            ApiClient apiClient = fireblocks.getApiClient();
            String baseUri = apiClient.getBaseUri();
            log.info("[resetClientByUriChange] baseUri:{}, baseUrl:{}", baseUri, baseUrl);
            if (!baseUri.equals(baseUrl)) {
                fireblocks = new Fireblocks(getConfigurationOptions());
            }
        }
    }
    ...
    public String getFileContent(String resPathName) {
        ...
        try {
            Resource resource = resourceLoader.getResource(resPathName);
...
//        OkHttpClient client = new OkHttpClient();
        ...
        String jwtToken = signJwt(builder, apiKey, signingAlgorithm); // 确保这个方法能够返回正确的JWT字符串
        ...
        HttpRequest.Builder requestBuilder = HttpRequest.newBuilder()
                ...
                .header("User-Agent", "fireblocks/sdk/java/2.0.0 (Windows 11 10.0; amd64)")
        ...
        if ("GET".equals(method)) {
            ...
            request = requestBuilder.POST(HttpRequest.BodyPublishers.ofString(json, StandardCharsets.UTF_8))
                    .build();
        }
    ...
    public String readPrivateKey(String keyName) {
        try {
            Resource resource = resourceLoader.getResource("file:/app/key/" + keyName + ".key");
