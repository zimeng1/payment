package com.mc.payment.fireblocksapi.impl;
...
import com.mc.payment.fireblocksapi.model.constant.FireBlocksConstant;
import com.mc.payment.fireblocksapi.model.req.fireBlocks.*;
import com.mc.payment.fireblocksapi.model.vo.fireBlocks.*;
import com.mc.payment.fireblocksapi.util.CommonUtil;
...
@Component
public class FireBlocksAPIImpl implements FireBlocksAPI {
    ...
    @Override
    public RetResult<VaultAccountVo> createAccount(@Valid CreateAccountReq req) {
    ...
    @Override
    public RetResult<CreateVaultAssetVo> createWallet(@Valid CreateWalletReq req) {
            ...
            if (StringUtils.isNotBlank(assetId) && assetId.contains("$")) {
                try {
                    ...
                    if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                        ...
                        CreateVaultAssetVo createVaultAssetVo = JSONUtil.toBean(result, CreateVaultAssetVo.class);
    ...
    @Override
    public RetResult<VaultAccountsPagedVo> queryAccountPage(@Valid QueryAccountReq req) {
//        log.info("[queryAccount] req :{}", req);
    ...
    @Override
    public RetResult<PaginatedAddressVo> queryAssetAddresses(@Valid QueryAssetAddressesReq req) {
//        log.info("[createAccount] vaultAccountId :{}, assetId:{}, req :{}", vaultAccountId, assetId, req);
        ...
        try {
            ...
            if (StringUtils.isNotBlank(assetId) && assetId.contains("$")) {
                try {
                    ...
                    if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                        PaginatedAddressVo vo = JSONUtil.toBean(result, PaginatedAddressVo.class);
                        return RetResult.data(vo);
                    } else {
    ...
    @Override
    public RetResult<VaultAssetVo> queryVaultAccountAsset(@Valid QueryVaultAccountAssetReq req) {
//    public RetResult<VaultAssetVo> queryVaultAccountAsset(@PathVariable("vaultAccountId") String vaultAccountId, @PathVariable("assetId") String assetId)
        ...
        try {
            ...
            if (StringUtils.isNotBlank(assetId) && assetId.contains("$")) {
                try {
                    ...
                    if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                        VaultAssetVo vo = JSONUtil.toBean(result, VaultAssetVo.class);
                        return RetResult.data(vo);
                    } else {
    ...
    public RetResult<VaultAccountVo> queryAccount(String vaultAccountId) {
        try {
            Fireblocks fireblocks = fireBlocksUtil.getFireBlocks();
            CompletableFuture<ApiResponse<VaultAccount>> result = fireblocks.vaults().getVaultAccount(vaultAccountId);
            try {
                ApiResponse<VaultAccount> response = result.get();
                VaultAccountVo vo = new VaultAccountVo();
                BeanUtil.copyProperties(response.getData(), vo);
                ...
                log.error("[queryAccount] API调度异常: vaultAccountId={},  errorMsg={}", vaultAccountId, e.getMessage());
                return RetResult.error(e.getMessage());
            }
        } catch (Exception e) {
            log.error("[queryAccount] has bean error, vaultAccountId:{}, assetId:{}", vaultAccountId, e);
            return RetResult.error();
        }
    }

    @Override
    public RetResult<List<TransactionVo>> queryTransactions(@Valid QueryTransactionsReq req) {
//        log.info("[queryTransactions] req :{}", req);
    ...
    @Override
    public RetResult<TransactionVo> queryTransactionsById(@Valid QueryTransactionsByIdReq req) {
//        log.info("[queryTransactionsById] txId :{}", txId);
    ...
    @Override
    public RetResult<CreateTransactionVo> createTransactions(@Valid CreateTransactionReq req) {
        ...
        try {
            ...
            //模拟测试接口, ps: fireblocks的交易接口在调度时mock测试有问题, 这里采用通用接口调度
            if (StringUtils.isNotBlank(baseUrl) && baseUrl.contains("localhost")) {
                return mockTest(req);
            }
     ...
     * @return
     */
    private RetResult<CreateTransactionVo> mockTest(CreateTransactionReq req) {
        try {
            String result = fireBlocksUtil.doFireBlocksApiByUrl("/transactions", "post");
            if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                CreateTransactionVo vo = JSONUtil.toBean(result, CreateTransactionVo.class);
                return RetResult.data(vo);
            } else {
                log.error("[createTransactions] API调度失败: req={}, errorMsg={}", req, result);
                return RetResult.error(result);
            }
        } catch (Exception e) {
            log.error("[createTransactions] API调度异常: req={}, errorMsg={}", req, e.getMessage());
            return RetResult.error(e.getMessage());
        }
    }
...
// }
    ...
    @Override
    public RetResult<EstimatedTransactionFeeVo> estimateFee(@Valid CreateTransactionReq req) {
...
//            TransactionRequest transactionRequest, String idempotencyKey
    ...
    @Override
    public RetResult<EstimatedNetworkFeeVo> estimateNetworkFee(@Valid QueryEstimateNetworkFeeReq req) {
        String assetId = req.getAssetId();
        try {
            ...
            if (StringUtils.isNotBlank(assetId) && assetId.contains("$")) {
                try {
                    ...
                    if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                        EstimatedNetworkFeeVo vo = JSONUtil.toBean(result, EstimatedNetworkFeeVo.class);
                        return RetResult.data(vo);
                    } else {
    ...
    @Override
    public RetResult<RegisterNewAssetVo> registerNewAsset(@Valid RegisterNewAssetReq req) {
        try {
     ...
     */
    @Override
    public RetResult<CreateVaultAssetVo> activateAsset(@Valid VaultAccountAndAssetReq req) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(assetId) && assetId.contains("$")) {
                try {
                    ...
                    if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                        CreateVaultAssetVo vo = JSONUtil.toBean(result, CreateVaultAssetVo.class);
                        return RetResult.data(vo);
                    } else {
     ...
     */
    @Override
    public RetResult<ResendWebhooksVo> resendWebhooks(@Valid ResendWebhooksReq req) {
        String txId = req.getTxId();
    ...
    /**
     * Refresh asset balance data 刷新资产余额
     ...
     */
    @Override
    public RetResult<VaultAssetVo> updateVaultAccountAssetBalance(@Valid VaultAccountAndAssetReq req) {
        log.info("[updateVaultAccountAssetBalance] req :{}", req);
        String vaultAccountId = req.getVaultAccountId();
        String assetId = req.getAssetId();
        try {
            Fireblocks fireblocks = fireBlocksUtil.getFireBlocks();
            ...
            if (StringUtils.isNotBlank(assetId) && assetId.contains("$")) {
                try {
                    ...
                    String result = fireBlocksUtil.doFireBlocksApiByUrl(String.format("/vault/accounts/%s/%s/balance", vaultAccountId, assetId), "POST");
                    if (StringUtils.isNotBlank(result) && !(result.contains("message") && result.contains("code"))) {
                        VaultAssetVo vo = JSONUtil.toBean(result, VaultAssetVo.class);
                        return RetResult.data(vo);
                    } else {
                        log.error("[updateVaultAccountAssetBalance] API调度失败: req={}, errorMsg={}", req, result);
                        return RetResult.error(result);
                    }
                } catch (Exception e) {
                    log.error("[updateVaultAccountAssetBalance] API调度异常: req={}, errorMsg={}", req, e.getMessage());
                    return RetResult.error(e.getMessage());
                }
            }
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyMMdd");
            String idempotencyKey = vaultAccountId + "_" + assetId + LocalDate.now().format(formatter);
            CompletableFuture<ApiResponse<VaultAsset>> result = fireblocks.vaults().updateVaultAccountAssetBalance(vaultAccountId, assetId, idempotencyKey);
            try {
                ApiResponse<VaultAsset> response = result.get();
                VaultAssetVo vo = new VaultAssetVo();
                BeanUtil.copyProperties(response.getData(), vo);
                ...
                log.error("[updateVaultAccountAssetBalance] API调度异常: vaultAccountId={}, assetId={}, errorMsg={}", vaultAccountId, assetId, e.getMessage());
                return RetResult.error(e.getMessage());
            }
        } catch (Exception e) {
            log.error("[updateVaultAccountAssetBalance] has bean error, vaultAccountId:{}, assetId:{}", vaultAccountId, assetId, e);
            return RetResult.error();
        }
    }
}
